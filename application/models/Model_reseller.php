...
class Model_reseller extends CI_model{
    ...
    function penjualan_list_konsumen($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen JOIN rb_penjualan_detail c ON a.id_penjualan=c.id_penjualan JOIN rb_produk d ON c.id_produk=d.id_produk where a.status_penjual='$level' AND a.id_penjual='$id' ORDER BY a.id_penjualan DESC");
    }
    ...
    function profile_konsumen($id){
        return $this->db->query("SELECT a.id_konsumen, a.username, a.nama_lengkap, a.email, a.jenis_kelamin, a.alamat_lengkap, a.kecamatan, a.kelurahan, a.no_hp, a.tanggal_daftar, b.kota_id, b.nama_kota as kota, c.provinsi_id, c.nama_provinsi as propinsi FROM `rb_konsumen` a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id LEFT JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id where a.id_konsumen='$id'");
    }
    function profile_reseller($id){
        return $this->db->query("SELECT a.id_reseller, a.username, a.nik, a.keterangan, a.nama_reseller, a.nama_pemilik, a.email, a.jenis_kelamin, a.alamat_lengkap, a.kecamatan, a.kelurahan, a.no_telpon, a.tanggal_daftar, b.kota_id, b.nama_kota as kota, c.provinsi_id, c.nama_provinsi as propinsi FROM `rb_reseller` a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id LEFT JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id where a.id_reseller='$id'");
    }
    ...
    function profile_update($id){
        if (trim($this->input->post('a')) != ''){
            $datadbd = array('username'=>$this->db->escape_str(strip_tags($this->input->post('aa'))),
                            ...
                            'kelurahan'=>$this->db->escape_str(strip_tags($this->input->post('i'))),
    ...
    function penjualan($id_reseller){
        return $this->db->query("SELECT sum((a.jumlah*a.harga_jual)+c.ongkir-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk
    ...
    function modal_perusahaan($id_reseller){
        return $this->db->query("SELECT sum(a.jumlah*b.harga_reseller) as total FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_pembeli='konsumen' AND c.proses='1' AND c.id_penjual='".$id_reseller."'");
    }
    ...
    function produk_perkategori($id_reseller,$id_produk_perusahaan,$id_kategori_produk,$limit){
        return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='$id_reseller' AND a.id_kategori_produk='$id_kategori_produk' ORDER BY a.id_produk DESC LIMIT $limit");
    }
