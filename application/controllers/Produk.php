...
class Produk extends CI_Controller {
	function index(){
		$jumlah= $this->db->query("SELECT * FROM rb_produk where id_reseller!='0'")->num_rows();
		...
		if (is_numeric($dari)) {
			if ($this->input->post('kata')){
				...
				$data['record'] = $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
	                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='0' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			}
	...
	function kategori(){
		...
		if (is_numeric($dari)) {
			...
			$this->pagination->initialize($config);
			$data['record'] = $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='0' AND a.id_kategori_produk='$cek[id_kategori_produk]' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			$this->pagination->initialize($config);
	...
	function subkategori(){
		...
		if (is_numeric($dari)) {
			...
			$this->pagination->initialize($config);
			$data['record'] = $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota FROM rb_produk a LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_reseller!='0' AND a.id_kategori_produk_sub='$cek[id_kategori_produk_sub]' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			$this->pagination->initialize($config);
			$this->template->load(template().'/template',template().'/reseller/view_kategori_all',$data);
		}else{
			redirect('main');
	...
	function reseller(){
		...
		if (is_numeric($dari)) {
			$data['title'] = 'Semua Daftar Pelaku UMKM';
			$data['description'] = description();
			$data['keywords'] = keywords();
			$this->pagination->initialize($config);
	...
	function keranjang(){
		...
		if ($id_produk!=''){
			if ($stok < $qty){
				...
				echo "<script>window.alert('Maaf, Stok untuk Produk $produk_cek pada Pelaku UMKM ini telah habis, silahkan menunggu atau order dengan pelaku UMKM lain!');
				...
				if ($this->session->reseller==$id_reseller){
					...
					$data['description'] = description();
					$data['keywords'] = keywords();
					echo $this->session->set_flashdata('message', '<div class="alert alert-danger"><center>Maaf, Dalam 1 Transaksi hanya boleh order dari 1 UMKM saja.</center></div>');
					redirect('produk/keranjang');
				}
			}
		}else{
