...
class Reseller extends CI_Controller {
	function index(){
		if (isset($_POST['submit'])){
			...
			$cek = $this->model_app->cek_login($username,$password,'rb_reseller');
			...
			if ($total > 0){
                ...
                echo $this->session->set_flashdata('message', '<div class="alert alert-danger"><center>Data belum di validasi oleh admin!!</center></div>');
                redirect('reseller/index');
            }
		}else{
			$data['title'] = 'Pelaku UMKM &rsaquo; Log In';
		...
		if (isset($_POST['submit2'])){
			...
			if ($cek >= 1){
				...
				$data = array('username'=>$this->input->post('a'),
							  ...
							  'nama_pemilik'=>$this->input->post('i'),
				...
				$message      = "<html><body>Selamat, Pada Hari ini tanggal $tglaktif<br> Bpk/Ibk <b>".$this->input->post('c')."</b> Sukses Mendafatar Sebagai reseller dengan nama_pemilik <b>".$ref['nama_reseller']."</b>...";
    ...
    function kategori_produk(){
        cek_session_reseller();
		// $data['record'] = $this->model_app->view_where('rb_kategori_produk','id_kategori_produk','DESC',array('id_reseller'=>$this->session->id_reseller));
		// $this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_kategori_produk/view_kategori_produk',$data);
        $data['record'] = $this->model_app->view_ordering('rb_kategori_produk','id_kategori_produk','DESC');
        $this->template->load('reseller/template','reseller/mod_kategori_produk/view_kategori_produk',$data);
    }
    ...
    function tambah_kategori_produk(){
        ...
        if (isset($_POST['submit'])){
            ...
            redirect('reseller/kategori_produk');
        }else{
            $this->template->load('reseller/template','reseller/mod_kategori_produk/view_kategori_produk_tambah');
        }
    }
    ...
    function edit_kategori_produk(){
        ...
        if (isset($_POST['submit'])){
            ...
            $data = array('rows' => $edit);
            $this->template->load('reseller/template','reseller/mod_kategori_produk/view_kategori_produk_edit',$data);
        }
    }
    ...
    function kategori_produk_sub(){
        ...
        $this->template->load('reseller/template','reseller/mod_kategori_produk/view_kategori_produk_sub',$data);
    }
    ...
    function tambah_kategori_produk_sub(){
        ...
        if (isset($_POST['submit'])){
            ...
            redirect('reseller/kategori_produk_sub');
        }else{
            $this->template->load('reseller/template','reseller/mod_kategori_produk/view_kategori_produk_tambah_sub');
        }
    }
    ...
    function edit_kategori_produk_sub(){
        ...
        if (isset($_POST['submit'])){
            ...
            $data = array('rows' => $edit);
            $this->template->load('reseller/template','reseller/mod_kategori_produk/view_kategori_produk_edit_sub',$data);
        }
    }
	...
	function detail_penjualan(){
		...
		$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','rb_produk','id_produk',array('id_penjualan'=>$this->uri->segment(3)),'id_penjualan_detail','DESC');
		// $data['kota'] = $this->model_app->view_join_where('rb_kota','rb_konsumen','kota_id',array('id_konsumen'=>$this->uri->segment(3)),'id_konsumen','DESC');
	...
	function proses_konfirmasi_pembayaran(){
		cek_session_reseller();
			...
			$where = array('id_penjualan' => $this->uri->segment(3));
			$this->model_app->update('rb_penjualan', $data, $where);
			redirect($this->uri->segment(1).'/pembayaran_konsumen');
	}
	...
	function pembayaran_konsumen(){
		cek_session_reseller();
		// $data['record'] = $this->model_app->view_where('rb_konfirmasi_pembayaran_konsumen',array('id_rekening_reseller'=>$this->session->id_reseller));
		// $data['record'] = $this->db->query("SELECT a.*, b.*, c.kode_transaksi, c.proses FROM `rb_konfirmasi_pembayaran_konsumen` a JOIN rb_rekening_reseller b ON a.id_rekening=b.id_rekening_reseller JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.id_pembeli='".$this->session->id_reseller."' AND c.status_penjual='reseller'");
		// $this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_konsumen/view_konsumen_pembayaran',$data);
		// $data = array('proses'=>$this->uri->segment(4));
		$data['record'] = $this->db->query("SELECT a.*, b.*, c.kode_transaksi, c.proses FROM `rb_konfirmasi_pembayaran_konsumen` a JOIN rb_rekening_reseller b ON a.id_rekening=b.id_rekening_reseller JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.id_penjual='".$this->session->id_reseller."' AND c.status_penjual='reseller'");
		$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_konsumen/view_konsumen_pembayaran',$data);
	   
	}

	function pesanan_diterima_konsumen(){
		cek_session_reseller();
		// $data['record'] = $this->model_app->view_where('rb_konfirmasi_pembayaran_konsumen',array('id_rekening_reseller'=>$this->session->id_reseller));
		// $data['record'] = $this->db->query("SELECT a.*, b.*, c.kode_transaksi, c.proses FROM `rb_konfirmasi_pembayaran_konsumen` a JOIN rb_rekening_reseller b ON a.id_rekening=b.id_rekening_reseller JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.id_pembeli='".$this->session->id_reseller."' AND c.status_penjual='reseller'");
		// $this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_konsumen/view_konsumen_pembayaran',$data);
		$data['record'] = $this->db->query("SELECT * FROM `rb_pesanan_diterima` a JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.id_penjual='".$this->session->id_reseller."' AND c.status_penjual='reseller'");
		$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_konsumen/view_pesanan_diterima',$data);
