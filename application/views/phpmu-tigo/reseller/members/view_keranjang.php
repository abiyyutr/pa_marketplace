...
<p class='sidebar-title text-danger produk-title'>Keranjang Saya</p>
<div class='col-md-8'>
<?php 
echo "<form action='".base_url()."members/selesai_belanja' method='POST'>";
  echo $error_reseller; 
  if ($this->session->idp == ''){
    echo "<center style='padding:15%'><i class='text-danger'>Wah Keranjang belanjaanmu kosong!!</i><br>
...
?>
      ...
      <?php 
        echo "<a class='btn btn-success btn-sm' href='".base_url()."members/produk_reseller/$rows[id_reseller]'>Lanjut Belanja</a>
              <a class='btn btn-danger btn-sm' href='".base_url()."members/batalkan_transaksi' onclick=\"return confirm('Yakin ingin membatalkan transaksi?')\">Batalkan Transaksi</a>"; 
      ?>
        ...
        <?php 
          ...
          echo "<tr><td>$no</td>
                    ...
                    <td><a style='color:#ab0534' href='".base_url()."produk/detail/$row[produk_seo]'><b>$row[nama_produk]</b></a>
                        <br>Qty : <b>$row[jumlah]</b><br>Harga : Rp ".rupiah($row['harga_jual']-$row['diskon'])." / $row[satuan]
                        <br>Berat : <b>".($row['berat']*$row['jumlah'])." Gram</b></td>
                    <td>Rp ".rupiah($sub_total)."</td>
                    <td width='30px'><a class='btn btn-danger btn-xs' title='Hapus' href='".base_url()."members/keranjang_delete/$row[id_penjualan_detail]'><span class='glyphicon glyphicon-remove'></span></a></td>
                </tr>";
            $no++;
          }
      ...
      <div class='col-md-4 col-xs-12 pull-right'>
        <center>Total Bayar <br><h2 id='totalbayar'></h2>   
          <button type='submit' name='submit' id='oksimpan' class='btn btn-success btn-flat btn-sm' style='display: none'>Buat Pesanan</button>
        </center>
      </div>";
...
<div class="form-group">
    <label class="col-sm-4 control-label" for="">Metode Pembayaran</label>
    
    <div class="col-md-10">
        <?php       
        $kurir=array('Transfer antar bank');
        foreach($kurir as $rkurir){
            ?>          
                <label class="radio-inline">
                <input type="radio" name="kurir" class="kurir" value="<?php echo $rkurir; ?>"/> <?php echo ucwords($rkurir); ?>
                </label>
            <?php
        }
        ?>
        <label class="radio-inline"><input type="radio" name="kurir" class="kurir" value="cod"/> COD (Bayar di tempat)</label>
...
<div class="col-sm-4 colom4">
  ...
  <?php $tes = $this->db->query("SELECT * FROM rb_keterangan 
                  where id_reseller='$rows[id_reseller]'")->row_array(); ?>
  <table class='table table-condensed'>
  <tbody>
  <?php 
        echo "
              <a class='btn btn-warning btn-sm' href='".base_url()."members/profile'>Ubah Alamat Pengiriman</a><br>"; 
      ?>
    
    <br><tr class='alert alert-info'><th scope='row' style='width:90px'>Informasi Toko</th> <td><?php echo $tes['keterangan']?></td></tr>
  </tbody>
  </table>
