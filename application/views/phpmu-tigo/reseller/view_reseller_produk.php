<p class='sidebar-title text-danger produk-title'>Detail Data Produk pelaku UMKM</p>
  ...
  <hr>
      ...
      if ($record->num_rows()<=0){
          echo "<center style='margin:40px 0px; font-weight:bold; color:#ab0534'>Belum ada Produk yang dijual pada UMKM ini</center>";
      }else{
        $no = 1;
        ...
        echo "<div class='col-md-2 col-xs-6 '>
                  <center>
                    ...
                    if ($beli['beli']-$jual['jual']<=0){
                      echo "<a class='btn btn-success btn-block btn-sm' href='#'>Beli Sekarang</a>";
                    }else{
                      if($this->session->level=='konsumen'){
                        echo "<a class='btn btn-success btn-block btn-sm' href='".base_url()."produk/detail/$row[produk_seo]'>Beli Sekarang</a>";
                      }else{
                        echo "<a class='btn btn-success btn-block btn-sm' href='".base_url()."produk/detail/$row[produk_seo]'>Beli Sekarang</a>";
                      }
                    }
